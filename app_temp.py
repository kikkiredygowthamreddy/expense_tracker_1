...
def get_guest_token_from_request():
    # Priority: header X-Guest-Token, then cookie
...
def set_guest_cookie(response, token):
    ...
    response.set_cookie(COOKIE_NAME, token, max_age=max_age, expires=expires, httponly=True, samesite="Lax")
    return response
...
def build_monthly_wide(db, guest_token):
    ...
    wide.index = pd.to_datetime(wide.index + "-01"); wide.index.name = "Date"
    return wide

def generate_insights_text(wide):
    if wide.shape[0] < 2: return ["Not enough months to generate insights."]
    last_month = wide.index.max(); prev_months = wide.index[:-1]
    last_vals = wide.loc[last_month]; prev_avg = wide.loc[prev_months].mean()
    insights=[]
    for cat in wide.columns:
        prev = prev_avg.get(cat, 0.0); last = last_vals.get(cat, 0.0)
        if prev==0 and last==0: continue
        pct = np.inf if prev==0 else (last - prev)/prev*100
        if pct==np.inf: insights.append(f"{cat}: New spending this month: ₹{last:.2f}.")
        elif pct>20: insights.append(f"{cat}: You spent {pct:.1f}% more than usual (₹{last:.2f} vs avg ₹{prev:.2f}).")
        elif pct < -20: insights.append(f"{cat}: You spent {abs(pct):.1f}% less than usual (₹{last:.2f} vs avg ₹{prev:.2f}).")
    if not insights: insights=["No notable changes this month."]
    return insights
...
def predict_next_month(wide):
    ...
    for cat in wide.columns:
        ...
        X = dfc[["month_idx"]].values; y = dfc[cat].values
        if len(dfc) < 4:
            preds[cat] = {"status":"not_enough_data"}; continue
        X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=0.2,shuffle=False)
        model = LinearRegression().fit(X_train, y_train)
        next_idx = np.array([[X.max() + 1]]); pred = float(model.predict(next_idx)[0])
...
@app.before_request
def before_request():
    db = SessionLocal(); cleanup_old_data(db); db.close()

# ---------- Routes ----------
@app.route("/", methods=["GET"])
def index():
    token = get_guest_token_from_request()
    # If no token, create and return in JSON (frontend will store it)
    if not token:
        token = create_guest_token()
    resp = jsonify({"message":"temporary expense tracker", "ttl_days": TTL_DAYS, "guest_token": token})
    # Also set cookie for same-site use if browser supports (optional)
...
def get_token():
    token = create_guest_token()
    resp = jsonify({"guest_token": token}); set_guest_cookie(resp, token)
    return resp
...
def report():
    token = get_guest_token_from_request()
    if not token: return jsonify({"error":"no guest token (upload first)"}), 400
    db = SessionLocal()
    wide = build_monthly_wide(db, token)
    db.close()
    if wide.empty: return jsonify({"error":"no expense data"}), 400
...
def export_csv():
    token = get_guest_token_from_request()
    if not token: return jsonify({"error":"no guest token"}), 400
    db = SessionLocal()
    rows = db.query(GuestExpense).filter(GuestExpense.guest_token==token).order_by(GuestExpense.date).all()
    db.close()
    if not rows: return jsonify({"error":"no data to export"}), 400
    out_df = pd.DataFrame([{"Date": r.date, "Category": r.category, "Amount": r.amount} for r in rows])
    buf = io.StringIO(); out_df.to_csv(buf, index=False); buf.seek(0)
    return send_file(io.BytesIO(buf.getvalue().encode("utf-8")), mimetype="text/csv", as_attachment=True, download_name="guest_expenses.csv")

@app.route("/clear", methods=["POST"])
def clear_data():
    token = get_guest_token_from_request()
    if not token: return jsonify({"status":"no token"}), 200
    db = SessionLocal()
...
# Optionally serve the static frontend index from backend when testing locally:
